document.addEventListener("DOMContentLoaded", function () {
  ...
  async function populateStatistics() {
    ...
    try {
      ...
      // 5. กรองข้อมูล
      const filteredCounts = allCounts.filter((item) => item.page_name !== "project-overview" && item.page_name !== "statistics");
  ...
  function loadContent(page) {
    ...
    fetch(`pages/${page}.html`)
      .then((response) => {
        if (!response.ok) {
          throw new Error(`ไม่พบไฟล์: ${page.html}`);
        }
  ...
  if (closeMenuBtn) {
    closeMenuBtn.addEventListener("click", toggleMobileSidebar);
  }
  if (collapseSidebarBtn) {
    collapseSidebarBtn.addEventListener("click", toggleDesktopCollapse);
  }
